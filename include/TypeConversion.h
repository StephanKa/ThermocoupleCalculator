...
namespace Internal
{
    ...
    [[nodiscard]] constexpr auto calculate(const auto& value)
    {
        ...
        else if constexpr ((std::is_same_v<T, TypeK> || std::is_same_v<T, TypeB>) &&
            (Target == Helper::Conversion::Volt))
        {
        ...
        else
        {
            ...
            {
                return conversion<typename T::VoltageToTemperature, Target>(value);
            }
            else
            {
                return conversion<typename T::TemperatureToVoltage, Target>(value);
            }
        }
    }
