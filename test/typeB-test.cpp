...
TEST_CASE("TypeB - Positive Voltage To Temperature = 0.5mV")
{
    ...
    REQUIRE(Approx(321.9).epsilon(EPSILON) == TypeB::VoltageToTemperature::calculate(voltage));
}

TEST_CASE("TypeB - Positive Voltage To Temperature = 2.2mV")
{
    const auto voltage = 2.2;
    REQUIRE(Approx(665.4).epsilon(EPSILON) == TypeB::VoltageToTemperature::calculate(voltage));
}

TEST_CASE("TypeB - Positive Temperature To Voltage = 321.9°C")
{
    const auto temperature = 321.9;
    REQUIRE(Approx(0.5).epsilon(EPSILON) == TypeB::TemperatureToVoltage::calculate(temperature));
}

TEST_CASE("TypeB - Positive Temperature To Voltage = 665.4°C")
{
    const auto temperature = 665.4;
    REQUIRE(Approx(2.2).epsilon(EPSILON) == TypeB::TemperatureToVoltage::calculate(temperature));
}