...
TEST_CASE("TypeR - Positive Voltage To Temperature = 4.096mV")
{
    ...
    REQUIRE(Approx(465.2).epsilon(EPSILON) == calculate<TypeR, TypeR::VoltageToTemperature>(voltage));
}

TEST_CASE("TypeR - Positive Voltage To Temperature = 10.0mV")
{
    const auto voltage = 10.0;
    REQUIRE(Approx(961.5).epsilon(EPSILON) == calculate<TypeR, TypeR::VoltageToTemperature>(voltage));
}

TEST_CASE("TypeR - Negative Voltage To Temperature = -0.1mV")
{
    const auto voltage = -0.1;
    REQUIRE(Approx(-19.994).epsilon(EPSILON) == calculate<TypeR, TypeR::VoltageToTemperature>(voltage));
}

TEST_CASE("TypeR - Positive Temperature To Voltage = 1111.016°C")
{
    const auto voltage = 1111.016;
    REQUIRE(Approx(12.0).epsilon(EPSILON) == calculate<TypeR, TypeR::TemperatureToVoltage>(voltage));
}

TEST_CASE("TypeR - Positive Temperature To Voltage = 721.652°C")
{
    const auto voltage = 721.652;
    REQUIRE(Approx(7.0).epsilon(EPSILON) == calculate<TypeR, TypeR::TemperatureToVoltage>(voltage));
}